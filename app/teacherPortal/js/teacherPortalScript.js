...
function getMessagesForChat_chatPage_teacher_pageNation(classCode, studentEmail, lastElement){
    ...
    if (user) {
      ...
      firebase.firestore().collection('Class-Chats').doc(classCode).collection('Students').doc(studentEmail).collection('Messages').orderBy('timestamp', 'desc').limit(5).startAfter(lastElement).get().then(snap => {
        ...
        <div class="message-component" style=" float: right; width: 100%"  >
          <div class="row"><div class="container" style="width: 100%"></div><p style="color: white; background-color: royalblue; border-radius: 20px 20px 0px 20px; margin-right: 30px; padding: 20px; "><strong>${user}</strong>${message}</p></div>
        </div>
        </div>
        `
  ...
  <div class="message-component" style= " float: left; min-width: 900px"  >
    <div class="row"><div class="container" style="width: 100%"></div><p style="color: black; background-color: #d8e6eb; border-radius: 20px 20px 20px 0px; margin-right: 30px; padding: 20px; "><strong>${user}</strong>${message}</p></div>
  </div>
  </div>
  `
